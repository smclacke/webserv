...
/*   Updated: 2025/01/21 15:29:18 by smclacke      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void httpHandler::plainText(void)
{
	...
	if (_request.loc.root.empty())
		filepath = "." + _server.getRoot() + _request.loc.upload_dir + _request.uri;
	else
		filepath = "." + _request.loc.root + _request.loc.upload_dir + _request.uri;
	std::ofstream outFile;
